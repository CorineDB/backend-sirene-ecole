...
class BulkCreateJourFerieRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'jours_feries.*.pays_id' => ['nullable', 'string', 'exists:pays,id'],
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            foreach ($joursFeries as $index => $jour) {
                ...
                if ($estNational && !$calendrierId && !$paysId) {
                    $validator->errors()->add(
                        "jours_feries.{$index}.pays_id",
                ...
                // Validations liées au calendrier
                if ($calendrierId) {
                    $calendrier = CalendrierScolaire::find($calendrierId);
                    if (!$calendrier) continue;

                    // 3. Vérifier que ecole appartient au même pays
                    if ($ecoleId) {
                ...
                if ($date) {
                    $query = JourFerie::where('date', $date);
                    if ($calendrierId) {
                        $query->where('calendrier_id', $calendrierId);
                    }
                    if ($ecoleId) {
